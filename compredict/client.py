...
from compredict.utils.authentications import generate_token, refresh_token, verify_token
...
class api:
    ...
    def refresh_token(self, token_to_refresh: str = None):
        """
        Takes a refresh type JSON web token and returns an access type JSON web token if the refresh token is valid.

        By calling this method, instance of the client is automatically updated with new access token.
        :param token_to_refresh: refresh_token generated by user with username and password
        """
        if token_to_refresh is None and self.token_refresh is None:
            raise ClientError("Please provide refresh token.")
        token = token_to_refresh if token_to_refresh is not None else self.token_refresh
        response = refresh_token(self.url, token)
        token = self.connection.handle_response(response, True)
        self.token = token['access']
    ...
    def verify_token(self, token: str = None) -> bool:
        ...
        :return: str message with result
        """
